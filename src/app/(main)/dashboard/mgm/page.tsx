...
export default function MGMDashboardPage() {
  ...
  useEffect(() => {
    if (status === 'authenticated' && session?.user?.role === 'MGM') {
      fetchReceipts();
  ...
  const fetchReceipts = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/receipts?status=PENDING_MGM'); // Filter for MGM pending receipts
      if (!res.ok) {
  ...
  return (
    <div style={{ padding: '24px' }}>
      {contextHolder}
      <Title level={2}>MGM Dashboard - Pending Receipts</Title>
